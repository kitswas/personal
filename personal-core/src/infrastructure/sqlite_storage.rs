...
pub struct SqliteStorage {
	...
	crdt_extension_path: Option<PathBuf>,
}
...
impl std::fmt::Debug for SqliteStorage {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("SqliteStorage")
			...
			.field("crdt_extension_path", &self.crdt_extension_path)
			.finish()
	}
}
...
impl SqliteStorage {
	pub fn new(db_path: PathBuf, encryption_key: String) -> Self {
		Self {
			db_path,
			encryption_key,
			crdt_extension_path: None,
		}
	}

	pub fn with_crdt_extension(mut self, ext_path: PathBuf) -> Self {
		self.crdt_extension_path = Some(ext_path);
		self
	}
	...
	fn get_connection(&self) -> Result<Connection, StorageError> {
		...
		// Load prebuilt cr-sqlite C extension if configured
		if let Some(ref ext_path) = self.crdt_extension_path {
			unsafe {
				conn.load_extension(ext_path, None::<&str>).map_err(|e| {
					StorageError::DbError(format!(
	...
	/// Query changes tracking table generated by cr-sqlite since a given db_version
	pub fn get_crdt_changes(
		&self,
		since_db_version: i64,
	) -> Result<Vec<u8>, StorageError> {
		let conn = self.get_connection()?;
		let mut stmt = conn
			.prepare("SELECT quote(site_id) FROM crsql_site_id")
			.map_err(|e| {
				StorageError::DbError(format!("cr-sqlite not initialized: {}", e))
			})?;

		let _site_id: String = stmt.query_row([], |r| r.get(0)).map_err(|e| {
			StorageError::DbError(format!("Failed to query site_id: {}", e))
		})?;

		// In actual sync usage, crsql_changes table rows would be serialized to JSON/CBOR
		...
		let rows = changes_stmt
			.query_map(params![since_db_version], |_row| Ok(()))
			.map_err(|e| StorageError::DbError(e.to_string()))?;

		let mut count = 0;
		for r in rows {
			r.map_err(|e| StorageError::DbError(e.to_string()))?;
			count += 1;
		}

		Ok(format!("{{\"changes_count\": {}}}", count).into_bytes())
	}
